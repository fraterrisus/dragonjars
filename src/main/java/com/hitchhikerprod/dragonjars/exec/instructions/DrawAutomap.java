...
public class DrawAutomap implements Instruction {
    ...
    public Address exec(Interpreter i) {
        ...
        final VideoBuffer automapBuffer = new VideoBuffer();
        automap = new VideoHelper(i.memory().getCodeChunk());
        automap.setVideoBuffer(automapBuffer);
    ...
    private Address displayAutomapPage(final Interpreter i, final int x0, final int y0) { // 0x16f0
        final PartyLocation loc = i.getPartyLocation();
        ...
        for (int boxy = 0; boxy <= 7; boxy++) { // top to bottom, plus one
            ...
            for (int boxx = 0; boxx <= 9; boxx++) { // left to right, plus one
                ...
                if (square.touched()) {
                    drawChunk(i, automap, square.floorTextureChunk(), 0, xOffset, yOffset);
                }
                ...
                if (square.touched() || squareLeft.touched()) {
                    square.westWallTextureChunk().ifPresent(id -> drawChunk(i, automap, id, 2, xOffset-8, yOffset-8));
                }
                ...
                if (square.touched() || squareUp.touched()) {
                    square.northWallTextureChunk().ifPresent(id -> drawChunk(i, automap, id, 0, xOffset-8, yOffset-8));
                }

                // Party avatar, or Deco (texture offset 0)
                if (mapx == loc.pos().x() && mapy == loc.pos().y()) {
                    automap.textureData(i.memory().getCodeChunk(), VideoHelper.LITTLE_MAN_TEXTURE_ADDRESS,
                            xOffset, yOffset, 0, automapRectangle);
                } else if (square.touched()) {
                    square.otherTextureChunk().ifPresent(id -> drawChunk(i, automap, id, 0, xOffset, yOffset));
                }
            }
        }
        ...
        i.app().setKeyHandler(event -> {
            switch(event.getCode()) {
                case KeyCode.ESCAPE -> i.doLater(j -> {
                    ...
                    j.resetUI();
                    j.start(j.getIP().incr(OPCODE));
                });
                case KeyCode.UP, KeyCode.I, KeyCode.A -> {
                    final int y1 = y0 + 1;
                    if (y1 < yMax) i.doLater(j -> displayAutomapPage(j, x0, y1));
                }
                case KeyCode.DOWN, KeyCode.K, KeyCode.Z -> {
                    final int y1 = y0 - 1;
                    if (y1 >= 0) i.doLater(j -> displayAutomapPage(j, x0, y1));
                }
                case KeyCode.RIGHT, KeyCode.L -> {
                    final int x1 = x0 + 1;
                    if (x1 < xMax) i.doLater(j -> displayAutomapPage(j, x1, y0));
                }
                case KeyCode.LEFT, KeyCode.J -> {
                    final int x1 = x0 - 1;
                    if (x1 >= 0) i.doLater(j -> displayAutomapPage(j, x1, y0));
                }
            }
        });
    ...
    private void drawChunk(Interpreter i, VideoHelper d, int chunkId, int chunkIndex, int x, int y) {
