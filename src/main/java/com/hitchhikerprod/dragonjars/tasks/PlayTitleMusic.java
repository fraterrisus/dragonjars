...
public class PlayTitleMusic extends Task<Void> {
    ...
    private void decodeTitleMusic() {
        ...
        int pointer = ExecutableLayout.getInstance().getTitleMusicAddress();
        final int durationLutAddress = pointer - 0x077e;
        final int frequencyLutAddress = pointer - 0x05df;
        ...
        while (true) {
            ...
            final int durationIndex = durationLutAddress + (durationByte & 0x1f);
            ...
            if (freq != 0x7f) {
                ...
                final int frequencyValue = codeChunk.getWord(frequencyLutAddress + (2 * frequencyIndex));
